...
const uploadImage = async (imagePath) => {
  ...
  if (!fs.existsSync(fullPath)) {
    console.warn(`⚠️ [WARNING] Imagen no encontrada localmente en: ${fullPath}`);
    return null;
  }
...
const runSeeder = async () => {
  ...
  if (!process.env.SANITY_API_TOKEN) {
    throw new Error("❌ FALTA EL TOKEN: SANITY_API_TOKEN no está definido en el archivo .env");
  }

  // 1. Mapeo de Settings (global.json)
  console.log("\n⚙️  Procesando Settings (global.json)...");
  const settingsRaw = fs.readFileSync(path.join(__dirname, 'src/content/settings/global.json'), 'utf-8');
  ...
  const settingsDoc = {
    ...
    ].filter(s => s.url).map((s, i) => ({ ...s, _key: 'soc_' + i }))
  };
  ...
  const homepageRaw = fs.readFileSync(path.join(__dirname, 'src/content/homepage/index.md'), 'utf-8');
  ...
  if (frontmatter.hero) {
    pageBuilder.push({
      ...
      buttons: frontmatter.hero.buttons?.map((b, i) => ({ ...b, _key: 'btn_'+i })) || [],
  ...
  if (frontmatter.banner && frontmatter.banner.length > 0) {
    const bannerItems = await Promise.all(frontmatter.banner.map(async (b, i) => ({
      _key: 'banner_'+i,
      title: b.title,
      subtitle: b.subtitle,
      icon: b.icon,
  ...
  if (frontmatter.we_apply) {
    const feature_list = await Promise.all(frontmatter.we_apply.feature_list.map(async (f, i) => ({
      _key: 'weapply_'+i,
      title: f.title,
      content: f.content,
      icon: f.icon,
  ...
  if (frontmatter.benefits) {
    const items = await Promise.all(frontmatter.benefits.items.map(async (b, i) => {
      let video = null;
      if (b.video) {
        video = {
  ...
  if (frontmatter.graph) {
    pageBuilder.push({
      ...
      items: frontmatter.graph.items?.map((i, idx) => ({
      ...
      future: (frontmatter.graph.future || frontmatter.graph.futuro)?.map((f, idx) => ({
        _key: 'future_'+idx,
        title: f.title,
        content: f.content
      })) || []
  ...
  if (frontmatter.application_types) {
    const items = await Promise.all(frontmatter.application_types.items.map(async (at, i) => ({
      _key: 'apptype_'+i,
      title: at.title,
      content: at.content,
  ...
  if (frontmatter.cta) {
    pageBuilder.push({
      ...
      buttons: frontmatter.cta.buttons?.map((b, idx) => ({ ...b, _key: 'cta_btn_'+idx })) || []
    });
  }
  ...
  console.log("🎉 ¡MIGRACIÓN COMPLETADA! Entra a Sanity Studio para ver tus datos.");
};
